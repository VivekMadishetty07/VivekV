(function( $ ) {
	...
	$.Shop.prototype = {
		init: function() {
			...
			this.cartPrefix = "shopp-"; // Prefix string to be prepended to the cart's name in the session storage
		...
		populateProducts:function(){
			...
			$.each(result, function() {
				$dropdown.append($("<option />").val(this.Category).text(this.Category));
			});
			...
			$.get(API+"/products", function(data, status){
				...
				$.each(prod, function(index) {
					$prodlist.append(
						'<li>'+
						'<div class="product-description" data-name="'+this.name+'" data-price="'+this.price+'">'+
							...
							'<form class="add-to-cart">'+
								...
								'<input type="text" name="product_id" id="product_id" class="product_id" hidden value="'+this.product_id+'" />'+
								'<input type="text" name="name" id="name" class="name" hidden value="'+this.name+'" />'+
								'<input type="number" name="price" id="price" class="price" hidden value="'+this.price+'" />'+
								'</div>'+
			...
			// var prod = $.grep(result, function(v) {
			...
			$('#cat').on('change', function() {
				...
				$.each(prod[0].Products, function(index) {
					$prodlist.append(
						'<li>'+
						'<div class="product-description" data-name="'+this.name+'" data-price="'+this.price+'">'+
							'<div class="product-image">'+
							...
							'</div>'+
							'<h3 class="product-name">'+this.name+'</h3>'+
							'<p class="product-id" hidden>'+this.product_id+'</p>'+
							'<p class="product-price">&#36; '+this.price+'</p>'+
							'<form class="add-to-cart">'+
								'<div>'+
									'<label for="qty-'+index+'">Quantity</label>'+
			...
			$(".add-to-cart").each(function() {
				$( this ).bind( "submit", function (event) {
					var storagelocal=sessionStorage
					event.preventDefault();
					var total = parseInt(storagelocal.getItem("shopp-total"));
					var qty=parseInt(event.target['qty'].value);
					var price= parseInt(event.target['price'].value) 
					var subTotal = qty * price;
					var sTotal = total + subTotal;
					storagelocal.setItem( "shopp-total", sTotal );
					var cart = storagelocal.getItem( 'shopp-cart' );
					var cartCopy =JSON.parse( cart );
					...
					storagelocal.setItem( 'shopp-cart', JSON.stringify( cartCopy ) );
					
					var shipping = parseInt(storagelocal.getItem( 'shopp-shipping-rates')) ;
					// var shippingRates = self._calculateShipping( qty );
					var shippingRates=1;
					var totalShipping = shipping + shippingRates;
					
					storagelocal.setItem( 'shopp-shipping-rates', totalShipping );
					alert("added to cart");
				} );
			});
				},1000)
